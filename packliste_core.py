...
def convert_file(input_path, output_path, user_dichtungen, show_message=True):
    ...
    if not os.path.isfile(template_path):
        raise FileNotFoundError(f"Template '{TEMPLATE_FILE}' wurde nicht gefunden unter: {template_path}")
    ...
    if show_message and not HEADLESS:
        messagebox.showinfo("Konvertierung abgeschlossen!", f"Datei wurde gespeichert unter:\n{output_path}")


# ----------------------------------------------------------------
# GUI-Start nur lokal, NICHT auf Render
# ----------------------------------------------------------------

def main():
    if HEADLESS or not _TK_AVAILABLE:
        print("GUI nicht verfügbar (HEADLESS oder kein tkinter).")
        return
    ...
    # Hier würdest du deinen bisherigen GUI-Aufbau wieder einsetzen
    root.mainloop()
